...
# Капитал и пробои — вневыборочные (скрипт 21, базельское правило с
# плюс-фактором): полновыборочная «экономия» была артефактом диагностики
# подгонки и в сводный рисунок не выносится (§ 3.7.4)
cap = pd.read_csv(TAB / "table30d_oos_capital_paths.csv",
                  parse_dates=[0], index_col=0)
period_bt = pd.read_csv(TAB / "table30b_oos_periods.csv")
...
# (d) Вневыборочный капитал (базельское правило с плюс-фактором)
ax4 = fig.add_subplot(gs[1, 3:])
ax4.plot(cap.index, cap["M0_EWMA"] / 1e9, color="#aaaaaa", lw=0.7, label="M0 EWMA")
...
for vs, ve, c in [("2020-02-20", "2020-06-30", "#7209b7"),
                  ("2022-02-21", "2022-06-30", "#f77f00")]:
    ax4.axvspan(pd.Timestamp(vs), pd.Timestamp(ve), alpha=0.10, color=c)
ax4.set_title("(г) Вневыборочный требуемый капитал: пиковая потребность FMSV ниже DCC-семейства",
              loc="left", fontweight="bold")
ax4.set_ylabel("млрд руб.")
ax4.legend(loc="upper right", frameon=False)

# (e) Вневыборочные пробои по подпериодам
ax5 = fig.add_subplot(gs[2, :])
piv = period_bt.pivot(index="period", columns="model", values="viol_%")
piv = piv.reindex(["2015-2019", "COVID", "Inter", "2022 shock", "Recent"])
...
ax5.set_title("(д) Вневыборочная частота пробоев VaR(99%) по подпериодам:  ожидаемый уровень — пунктир 1%",
              loc="left", fontweight="bold")
ax5.legend(loc="upper right", frameon=False, ncol=4)

fig.suptitle("Структурный сдвиг 2022–2026 и вневыборочная валидация моделей риска: интегральный обзор",
