...
def fevd_bk(B_list, Sigma, bands=None, ngrid=20000):
    ...
    ngrid — число узлов сетки частот. Спектральная плотность лог-волатильностей
    имеет острый пик у нуля (персистентные ряды), и на грубой сетке (ngrid=200)
    ошибка интегрирования достигала ~2 п.п. в пользу короткой полосы;
    ngrid=20000 даёт совпадение с эталонной реализацией frequencyConnectedness
    с точностью до сотых долей п.п. (сверка — скрипт 22_bk_crosscheck.R)."""
