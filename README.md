...
- Вневыборочный блок FMSV (`20_fmsv_oos.R`) контролирует сходимость каждой
  цепи формализованным критерием (константы `SD_BLOWUP`, `BLOWUP_SHARE_MAX`,
  `MAX_ATTEMPTS` в начале скрипта) и при необходимости перезапускает
  переоценку с новым зерном и удлинённым прогревом; зерно детерминировано
  парой «окно — попытка», результат воспроизводим; журнал всех попыток
  пишется в `output/oos_fmsv_retry_log_chunk*.csv`.
